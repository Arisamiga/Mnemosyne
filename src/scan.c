...
void addToList(char *name, ULONG size, STRPTR format) {
    ...
    if (EnableGraphOption) {
        ...
        image1 = BitMapObject, BITMAP_BitMap, bm, BITMAP_Width, bmpW,
            ...
            if (fib->fib_DirEntryType > 0) {
                ...
                if (listGadget) {
                    ...
                    if (totalSize < oldTotalSize) {
                        ...
                        oldTotalSize =
                            devideByGivenFormat(oldTotalSize, currentFormat);
                    }
                    // printf("Total: %ld - %ld = %ld\n", totalSize,
                    // oldTotalSize, totalSize - oldTotalSize);
                    if ((long)(totalSize - oldTotalSize) < 0
                        || currentFormat == 0 || NoRoundOption == FALSE) {
                        struct Values format = correctFormat(
                            totalSize - oldTotalSize, currentFormat);
