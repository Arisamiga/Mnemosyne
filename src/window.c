...
void UpdateMenu(struct Window *intuiwin, BOOL enabled) {
    ...
    if (visualInfo = GetVisualInfo(screen, NULL)) {
        if (menu =
                CreateMenus(MenuArray, GTMN_SecondaryError, &error, TAG_DONE)) {
            if (LayoutMenus(
                    menu, visualInfo, GTMN_NewLookMenus, TRUE, TAG_DONE)) {
...
void setFileSequence(struct Window *intuiwin,
    ...
    if (!lock) {
        ...
        updateBottomText(
            bottomText, windowObject, "Invalid Path, Select a valid path");
        FreeVec(path);
        return;
    }
    UnLock(lock);
...
void scanningSequence(int type,
    ...
    updateBottomTextW2Text(
        bottomText, windowObject, "Scanning: ", parentName, FALSE);
...
void createWindow(char *Path) {
    ...
    LISTBROWSER_ColumnInfo, &ciWithoutColours, LISTBROWSER_ColumnTitles, TRUE,
    LISTBROWSER_MultiSelect, FALSE, LISTBROWSER_Separators, TRUE,
    LISTBROWSER_ShowSelected, FALSE, LISTBROWSER_TitleClickable, TRUE,
    ListBrowserEnd;
    ...
    if (EnableGraphOption) {
        ...
        listBrowser = (struct Gadget *)ListBrowserObject, GA_ID, OID_MAIN_LIST,
        GA_RelVerify, TRUE, GA_Disabled, TRUE, LISTBROWSER_Labels, &contents,
        LISTBROWSER_ColumnInfo, &ciWithColours, LISTBROWSER_ColumnTitles, TRUE,
        LISTBROWSER_MultiSelect, FALSE, LISTBROWSER_Separators, TRUE,
        LISTBROWSER_ShowSelected, FALSE, LISTBROWSER_TitleClickable, TRUE,
        ListBrowserEnd;
    }
