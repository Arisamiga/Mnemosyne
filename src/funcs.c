...
void getNameFromPath(char *path, char *result, unsigned int resultSize) {
    ...
    if (pathLock) {
        struct FileInfoBlock *FIblock = (struct FileInfoBlock *)AllocVec(
...
void initializeIconTooltypes(void) {
    ...
    if (IconBase) {
        ...
        if (diskObj) {
            ...
            for (STRPTR *tool_types = diskObj->do_ToolTypes;
                 (buf = *tool_types);
                 ++tool_types) {
                // printf("%s\n", buf);
...
ULONG getBitmapColorPen(struct BitMap *bm) {
    ...
    ULONG pen = ((byte3 & 0x80) >> 7) * 8 | ((byte2 & 0x80) >> 7) * 4
                | ((byte1 & 0x80) >> 7) * 2 | ((byte0 & 0x80) >> 7);

    return pen > 0 ? pen : 1;
}

// Collect all node percentages from the list browser WITH COLORS from column 0
// images
//
...
ULONG collectAllNodePercentages(
    ...
    ULONG nodeCount = 0;
    struct Node *node;
    for (node = nodeList->lh_Head; node->ln_Succ && nodeCount < maxNodes;
         node = node->ln_Succ) {
        ...
        if (percentText) {
            ...
            if (image) {
                ...
                if (imgBitMap) {
                    nodeArray[nodeCount].colorPen =
                        getBitmapColorPen(imgBitMap);
                } else {
...
void updateBottomTextW2Text(Object *bottomText,
    ...
    if (Refresh && windowObject) {
        ...
        // Safely extract the real Intuition window pointer from your window
        // object
        GetAttr(WINDOW_Window, windowObject, (ULONG *)&intuiWindow);

        // If the window is currently open on screen, render ONLY this specific
        // gadget
        if (intuiWindow) {
